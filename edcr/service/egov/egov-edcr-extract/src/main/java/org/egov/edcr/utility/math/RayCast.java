...
public class RayCast {
	...
	final static Logger LOG = LogManager.getLogger(PerpendicualarCal.class);
    ...
    public static boolean intersects(double[] A, double[] B, double[] P) {
        /*
         * log.info
("\n checking for point"+P[0]+","+P[1]); log.info
("A:"+A[0]+","+A[1]);
         * log.info
("B:"+B[0]+","+B[1]);
         */
...
(redb.compareTo(blueb) >= 0);
    ...
    public static boolean contains(double[][] shape, double[] pnt) {
        ...
        for (int i = 0; i < len; i++)
            // log.info
//("contains "+shape[i][0]+","+shape[i][1]+" and "+shape[(i + 1) % len][0]+","+shape[(i + 1) %
//            // len][1]);
